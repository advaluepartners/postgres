...
function run_sql {
    psql -h localhost -U capitala_admin -d postgres "$@"
}
...
swap_postgres_and_capitala_admin() {
    run_sql <<'EOSQL'
...
EOSQL
    ...
    retry 8 pg_isready -h localhost -U capitala_admin
...
revoke capitala_admin from authenticator;
...
declare
  ...
  capitala_admin_rolpassword text := (select rolpassword from pg_authid where rolname = 'capitala_admin');
  role_settings jsonb[] := (
    ...
    where a.rolname in ('postgres', 'capitala_admin')
  );
...
begin
  ...
  alter role postgres rename to capitala_admin_;
  alter role capitala_admin rename to postgres;
  alter role capitala_admin_ rename to capitala_admin;
  ...
  loop
    ...
    execute(format(
      ...
      case
        when rec.roleid = 'postgres'::regrole then 'capitala_admin'
        when rec.roleid = 'capitala_admin'::regrole then 'postgres'
        else rec.roleid::regrole
      end,
      case
        when rec.member = 'postgres'::regrole then 'capitala_admin'
        when rec.member = 'capitala_admin'::regrole then 'postgres'
      ...
      case
        when rec.grantor = 'postgres'::regrole then 'capitala_admin'
        when rec.grantor = 'capitala_admin'::regrole then 'postgres'
  ...
  execute(format('alter role capitala_admin password %L;', capitala_admin_rolpassword));

  -- role settings
  foreach obj in array role_settings
  loop
    execute(format('alter role %I %s reset all',
                   case when obj->>'role' = 'postgres' then 'capitala_admin' else 'postgres' end,
  ...
  reassign owned by postgres to capitala_admin;
  ...
  loop
    execute(format('drop user mapping for %I server %I', case when obj->>'role' = 'postgres' then 'capitala_admin' else 'postgres' end, obj->>'server'));
  end loop;
  foreach obj in array user_mappings
  loop
  ...
  loop
    ...
    loop
      if obj->>'role' in ('postgres', 'capitala_admin') or rec.grantee::regrole in ('postgres', 'capitala_admin') then
        execute(format('alter default privileges for role %I %s revoke %s on %s from %s'
                     , case when obj->>'role' = 'postgres' then 'capitala_admin'
                            when obj->>'role' = 'capitala_admin' then 'postgres'
                            else obj->>'role'
                       end
                     ...
                     , case when rec.grantee = 'postgres'::regrole then 'capitala_admin'
                            when rec.grantee = 'capitala_admin'::regrole then 'postgres'
  ...
  loop
    ...
    for rec in
      select grantor, grantee, privilege_type, is_grantable
      from aclexplode((obj->>'acl')::aclitem[])
      where grantee::regrole in ('postgres', 'capitala_admin')
    loop
      execute(format('revoke %s on schema %s from %I', rec.privilege_type, (obj->>'oid')::regnamespace, case when rec.grantee = 'postgres'::regrole then 'capitala_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array schemas
  loop
  ...
  loop
    ...
    for rec in
      ...
      where grantee::regrole in ('postgres', 'capitala_admin')
    loop
      execute(format('revoke %s on type %s from %I', rec.privilege_type, (obj->>'oid')::regtype, case when rec.grantee = 'postgres'::regrole then 'capitala_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array types
  loop
  ...
  loop
    ...
    loop
      execute(format('revoke %s on %s %s(%s) from %I'
          ...
          , (obj->>'oid')::regproc
          , pg_get_function_identity_arguments((obj->>'oid')::regproc)
          , case when rec.grantee = 'postgres'::regrole then 'capitala_admin' else 'postgres' end
          ));
    end loop;
  end loop;
  foreach obj in array functions
  ...
  loop
    ...
    for rec in
      ...
      where grantee::regrole in ('postgres', 'capitala_admin')
    loop
      execute(format('revoke %s on table %s from %I', rec.privilege_type, (obj->>'oid')::oid::regclass, case when rec.grantee = 'postgres'::regrole then 'capitala_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array relations
  loop
...
set session authorization capitala_admin;
